...
class tobii_controller:
    ...
    def show_status(self, text_color='white', enable_mouse=False):
        ...
        while b_show_status:
            ...
            if self.gaze_data_status is not None:
                lp, lv, lgp, lgv, rp, rv, rgp, rgv = self.gaze_data_status
                msgst = 'Left origin: {:.3f},{:.3f},{:.3f} {}\n'.format(*lp, lv)
                msgst += 'Right origin: {:.3f},{:.3f},{:.3f} {}\n'.format(*rp, rv)
                msgst += 'Left gaze: {:.3f},{:.3f} {}\n'.format(*lgp, lgv)
                msgst += 'Right gaze: {:.3f},{:.3f} {}'.format(*rgp, rgv)
                msg.setText(msgst)

                if self._is_valid_tobii_value(lv) or self._has_finite_values(lp, expected_len=3):
                    leye.setPos(((lp[0]-0.5)/2,(lp[1]-0.5)/2))
                    radius = (1-lp[2])/2 if np.isfinite(lp[2]) else 0.025
                    leye.setRadius(max(0.01, min(0.05, radius)))
                    leye.draw()
                elif self._is_valid_tobii_value(lgv) or self._has_finite_values(lgp, expected_len=2):
                    leye.setPos(((lgp[0]-0.5)/2,(0.5-lgp[1])/2))
                    leye.setRadius(0.025)
                    leye.draw()

                if self._is_valid_tobii_value(rv) or self._has_finite_values(rp, expected_len=3):
                    reye.setPos(((rp[0]-0.5)/2,(rp[1]-0.5)/2))
                    radius = (1-rp[2])/2 if np.isfinite(rp[2]) else 0.025
                    reye.setRadius(max(0.01, min(0.05, radius)))
                    reye.draw()
                elif self._is_valid_tobii_value(rgv) or self._has_finite_values(rgp, expected_len=2):
                    reye.setPos(((rgp[0]-0.5)/2,(0.5-rgp[1])/2))
                    reye.setRadius(0.025)
                    reye.draw()
    ...
    def _is_valid_tobii_value(self, validity):
        if isinstance(validity, (bool, np.bool_)):
            return bool(validity)

        try:
            return int(validity) == 1
        except (TypeError, ValueError):
            validity_text = str(validity).upper()
            return 'INVALID' not in validity_text and validity_text.endswith('VALID')

    def _has_finite_values(self, values, expected_len):
        try:
            values = np.array(values, dtype=np.float64)
        except (TypeError, ValueError):
            return False

        return len(values) >= expected_len and np.all(np.isfinite(values[:expected_len]))
    ...
    def on_gaze_data_status(self, gaze_data):
        ...
        lgp = gaze_data.left_eye.gaze_point.position_on_display_area
        lgv = gaze_data.left_eye.gaze_point.validity
        rp = gaze_data.right_eye.gaze_origin.position_in_track_box_coordinates
        rv = gaze_data.right_eye.gaze_origin.validity
        rgp = gaze_data.right_eye.gaze_point.position_on_display_area
        rgv = gaze_data.right_eye.gaze_point.validity
        self.gaze_data_status = (lp, lv, lgp, lgv, rp, rv, rgp, rgv)
